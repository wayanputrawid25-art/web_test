<div>
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ $myAssignments ? 'My Stock Opnames' : 'Stock Opnames' }}</h1>
                <p class="text-gray-600 mt-1">Manage stock opname sessions</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="flex items-end gap-2">
                <label class="flex items-center">
                    <input type="checkbox" wire:model.live="myAssignments" class="w-4 h-4 text-indigo-600 border-gray-300 rounded">
                    <span class="ml-2 text-sm text-gray-700">My Assignments</span>
                </label>
            </div>
        </div>
        @if($search || $statusFilter || $myAssignments)
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($sessions as $session)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <a href="{{ route('stock_opnames.show', $session->id) }}" class="block">
                                <p class="font-medium text-gray-900 hover:text-indigo-600">{{ $session->code }}</p>
                                <p class="text-sm text-gray-500 mt-1">{{ $session->name }}</p>
                            </a>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $session->createdAt?->format('M d, Y') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="{{ route('stock_opnames.show', $session->id) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                            @can('edit-stock-opnames')
                                @if($session->status->isCountingAllowed())
                                    <a href="{{ route('stock_opnames.count', $session->id) }}" class="text-green-600 hover:text-green-900 mr-3">Count</a>
                                @endif
                            @endcan
