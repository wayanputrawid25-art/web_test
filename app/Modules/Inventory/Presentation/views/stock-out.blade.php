<div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h2 class="text-lg font-semibold text-gray-900">New Stock Out</h2>
                </div>
                <form wire:submit="store" class="p-6 space-y-4">
                    ...
                    <div>
                        ...
                        @if($selectedProduct)
                            @php
                                $balance = $stockBalances->firstWhere('product_id', $selectedProduct['id']);
                            @endphp
                            <div class="flex items-center justify-between p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                <div>
                                    ...
                                    <p class="text-xs text-blue-600 mt-1">Available: {{ $balance['current_stock'] ?? 0 }}</p>
                                </div>
                            ...
                            @if($searchProduct && count($products) > 0)
                                <div class="mt-2 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                                    @foreach($products as $product)
                                        @php
                                            $bal = $stockBalances->firstWhere('product_id', $product['id']);
                                        @endphp
                                        <button type="button"
                                            ...
                                            <div class="flex justify-between">
                                                ...
                                                <span class="text-sm text-gray-500">Stock: {{ $bal['current_stock'] ?? 0 }}</span>
                                            </div>
                                        </button>
                                    @endforeach
                                </div>
                    ...
                    <div class="flex justify-end pt-4">
                        <button type="submit"
                                ...
                                class="px-6 py-2 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 transition-colors disabled:opacity-50">
                            <span wire:loading.remove wire:target="store">Save Stock Out</span>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-semibold text-gray-900">Recent Stock Out</h2>
            </div>
                ...
                @forelse($recentTransactions as $transaction)
                    <div class="py-3 border-b border-gray-100 last:border-0">
                        <div class="flex justify-between items-start">
                            ...
                            <span class="text-red-600 font-medium">-{{ $transaction->quantity }}</span>
                        </div>
                        <p class="text-xs text-gray-400 mt-1">{{ $transaction->createdAt?->format('M d, Y H:i') }}</p>
                    </div>
                @empty
                    <p class="text-gray-500 text-center py-4">No recent stock out</p>
                @endforelse
            </div>
        </div>
    </div>
